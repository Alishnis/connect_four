      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4">
        ...
        <GlowCard accentColor="magenta" className="mb-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="font-mono text-sm text-[#E0E0E0]/50">
                {profile?.city ? `📍 ${profile.city}` : "🌐 Global"} · {profile?.is_pro ? "⚡ PRO" : "Free tier"}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6" style={{ borderTop: "1px solid #2D1B4E" }}>
            {[
              { label: "Wins", value: wins, color: "#00FFFF" },
              { label: "Losses", value: losses, color: "#FF2D78" },
              { label: "Total", value: total, color: "#FF9900" },
            ].map(s => (
        ...
        <GlowCard accentColor="cyan" className="!p-0 overflow-hidden">
          ...
          {!matches?.length ? (
            <div className="p-12 text-center font-mono text-[#E0E0E0]/40 uppercase tracking-widest">
              No matches yet. <Link href="/play" className="text-[#00FFFF] hover:underline">Play now →</Link>
            </div>
          ) : (
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid #2D1B4E" }}>
                  <th className="px-4 py-3 font-mono text-xs text-[#E0E0E0]/40 uppercase tracking-widest text-left">Date</th>
                  <th className="px-4 py-3 font-mono text-xs text-[#E0E0E0]/40 uppercase tracking-widest text-left">Mode</th>
                  <th className="px-4 py-3 font-mono text-xs text-[#E0E0E0]/40 uppercase tracking-widest text-left">Result</th>
                  <th className="px-4 py-3 font-mono text-xs text-[#E0E0E0]/40 uppercase tracking-widest text-right">Moves</th>
                </tr>
              </thead>
              <tbody>
                {matches.map(m => {
                  const result = m.winner_id === user.id ? "WIN" : m.winner_id ? "LOSS" : "DRAW";
                  const resultColor = result === "WIN" ? "#00FFFF" : result === "LOSS" ? "#FF2D78" : "#FF9900";
                  return (
                    <tr key={m.id} style={{ borderBottom: "1px solid #2D1B4E11" }}
                      ...
                      <td className="px-4 py-3 font-mono text-xs uppercase" style={{ color: "#E0E0E0" }}>
                        {m.is_vs_ai ? `vs AI [${m.ai_difficulty}]` : m.room_id ? "Multiplayer" : "Local"}
                      </td>
